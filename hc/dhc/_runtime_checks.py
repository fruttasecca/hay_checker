...
def _columns_index_to_name(columns, df):
    """
    ...
    idx_map = dict(enumerate(df.columns))
    res = []
    for col in columns:
        if type(col) is int:
            assert 0 <= col < len(df.columns), "Column index '%s' out of range" % col
            res.append(idx_map[col])
        else:
            res.append(col)
    return res


def _conditions_columns_index_to_name(conditions, df):
    """
    returns conditions where column indexes have been substituted by names, while column names (strings) will
    be untouched.
